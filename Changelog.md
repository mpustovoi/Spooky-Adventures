...
- **Distant Shaded Panoramas** (набор ресурсов) — Надземный мир, ночь, дождливая погода.
...
- **Timeless and Classics Guns: LesRaisins Append Pack** — Оружейный набор для **[TaCZ] Timeless and Classics Zero** в виде мода.
- **Toni's Immersive Lanterns** — Теперь источник света можно поместить в специальный слот.
- **Traveler's Titles**
- **YUNG's Cave Biomes**

### 🔁 Обновлено:
- ⚠ Возвращёна механика температуры из **Tough As Nails**. Значительно расширены списки предметов в тегах охлаждающих/подогревающих блоков, холодного/горячего топлива для терморегулятора, предметов для удержания охлаждения/нагрева, брони и её отделок, а также еды и напитков. Теперь жажду в различном объёме восстанавливают почти всё из следующего списка: зелья, вода, молоко, соки, чаи, кофе, коктейли, мороженое, фруктовое мороженое, джемы, желе, кремы, ягоды, арбузы, тыквы, торты, супы и алкогольные напитки; это распространяется на еду и напитки из **Farmer's Delight** и его аддонов, а также из нескольких других модов.
- ⚠ Обновлён набор данных **RPG-Loot**. Обновлён формат и исправлены многие ошибки в таблицах добычи, унаследованных ещё с `1.16`. Удалены неиспользуемые и дублирующиеся таблицы. `Очень редкие` предметы переименованы в `Эпические`. Перебалансированы некоторые предметы. Пересмотрено распределение таблиц по сундукам ванильных структур. Предметы соответствующей ценности по-прежнему можно найти в большинстве из ванильных структур, а также, где используются ванильные таблицы добычи сундуков. Эти предметы также можно получить при убийстве `разорителей` (1-2 случайных предмета), `кроликов-убийц` (1-2 предмета), `зомби-гигантов` (1-3), `древних стражей` (1-4), `хранителей` (2-4), `иссушителей` (6-12, без дублирующейся звезды Незера) и `Эндер-драконов` (8-16) (в будущем список будет пополнен некоторыми мобами из модов). Помимо предметов, теперь можно находить и особые зелья, которые раньше существовали, но не использовались. Структуры из **Battle Towers** являются отличным источником этих предметов. Все названия и описания локализованы на русский язык.
- Восстановлен встроенный набор оружия из **[TaCZ] Timeless and Classics Zero**. (Упс... чистка смела то, что не следовало)
...
- Исправлены дублирующиеся названия модов на всплывающих подсказках **EMI**. Исправлен периодический вылет при использовании функции быстрого создания с помощью Shift+клика по панели **EMI**. Изменена конфигурация по умолчанию.
- Немного оптимизирован конфиг **Sculk Horde**. Распространение замедлено с двадцатой доли до четверти секунды.
- Обновлены иконки ванильных мобов.
- Прочие небольшие изменения в конфигах.
- Скорректирован конфиг **The Hordes**. В список заражаемых зомби-инфекцией мобов добавлены все ванильно-выглядящие эндермены, а также разбойники из **It Takes a Pillage**, Некромант из **Illager Invasion** убран оттуда. Теперь крестьяне могут превратиться в зомби на любом уровне сложности. Сильно снижена длительность эффекта `Иммунитет` на еде (теперь она равна длительности эффекта `Регенерации`). Возвращены зомби-игроки. Полностью убраны подсказки о проценте устойчивости к зомби-инфекции на предметах брони, потому что они появляются просто везде.
...
- Теперь меню графики стало ещё более конфигурируемым, благодаря интеграциям нового **Sodium Options API**.
- Убрано рентгеновское зрение у криперов. Взамен увеличена их дальность видимости.
- Удалено множество наследных рецептов, которые не имели смысла и/или были слишком несбалансированы.
- Удорожены рецепты боеприпасов в **MrCrayfish's Gun Mod**. Теперь они дороже, чем раньше, но всё равно дешевле в 1,5-2 раза, чем в **[TaCZ] Timeless and Classics Zero**.
- Упразднён отдельный набор данных с дополнительными тегами (теперь он часть **Uncraftables**).
В очередной раз изменена генерация рудных жил (на этот раз в сторону увеличения).

### ⚠ Примечания и известные проблемы:
- ⚠ При выборе какого-либо из шейдеров **Complementary + Euphoria Patches** возможен сбой, при котором происходит наложения чанков детализации **Distant Horizons** поверх ванильных чанков. Проблема может быть решена при переключении на другой набор шейдеров и обратно, либо же сбросом настроек шейдера и последующим их восстановлением.
- ¹ В связи с добавлением мода **Euphoria Patches** все имеющиеся в модпаке перезаписанные файлы шейдеров (также включая **Rethinking Voxels**) были сброшены и теперь загружаются с **Modrinth**. Это означает, что при использовании шейдеров **Rethinking Voxels**, **Complementary Reimagined** или **Complementary Unbound** все рудные блоки из модов перестанут иметь светящиеся текстуры. Однако, при использовании шейдеров с **Euphoria Patches** многие светящиеся руды останутся.
- В некоторых рецептах в **EMI**, где присутствуют теги, показываются технические строки вроде `#modid:item_group` без какой-либо локализации. (Я не вижу способов как-то организованно локализовать это, кроме как вручную, чего я делать не хочу из-за невероятно огромного количества таких тегов)
- - Мод **Fog** не будет возвращён, так как поддержка 1.20.1 прекращена, поэтому у него уже не появится поддержка **Distant Horizons**.
- На данный момент отображение оружия из **[TaCZ] Timeless and Classics Zero** от первого лица сломано из-за вновь появившейся несовместимости с **First-person Model**. В качестве временного решения я внёс общий ID всего огнестрельного оружия из этого мода (`tacz:modern_kinetic_gun`) в список исключений, чтобы держание оружия было 'Ванильным'.
- Стартовый набор не выдаётся автоматически новым игрокам, присоединяющимся к локальному миру.

### 📋 Долгосрочные планы
- Возможно, в будущем могут быть добавлены **Quark** и **The Aether** с его популярными аддонами.
...
- **Delightful Burgers**
- **Delightful Sandwiches**
- **Distant Shaded Panoramas** (a resourcepack) — Overworld, night, rainy weather.
...
- **Supplementaries**
- **Third Person Shooting: Zero**
- **Timeless and Classics Guns: LesRaisins Append Pack** — A gun pack for **[TaCZ] Timeless and Classics Zero** as a mod.
- **Toni's Immersive Lanterns** — Now the light source can be placed in a special slot.
- **Traveler's Titles**
- **YUNG's Cave Biomes**

### 🔁 Updated:
- ⚠ Re-enabled the temperature from **Tough As Nails**. Significantly expanded the lists of items in the tags of cooling/heating units, cold/hot fuel for the thermoregulator, items for holding cooling/heating, armor and its finishes, as well as food and beverages. Now, thirst is restored in various amounts by almost everything from the following list: potions, water, milk, juices, teas, coffees, shakes, ice creams, popsicles, jams, jellies, custards, berries, watermelons, pumpkins, cakes, soups and alcoholic beverages; this applies to food and drinks from **Farmer's Delight** and its addons, as well as from several other mods.
- ⚠ Updated the **RPG-Loot** data pack. Updated the format and fixed many errors in the legacy loot tables inherited since `1.16`. Removed unused and duplicate tables. Renamed `Very Rare` items to `Epic` items. Rebalanced some items. Revised the distribution of tables by vanilla structures' chests. Items of respective value can still be found in most of the vanilla structures, as well as where vanilla chests' loot tables are used. These items can also be obtained by killing `Ravagers` (1-2 random items), `Killer Rabbits` (1-2 items), `Giant Zombies` (1-3), `Elder Guardians` (1-4), `Wardens` (2-4), `Withers` (6-12, without a Nether Star duplicate) and `Ender Dragons` (8-16) (in the future, the list will be expanded with some modded mobs). In addition to items, you can now find special potions that previously existed but were unused. The structures from **Battle Towers** are a great source of these items.
- Сhanged the generation of ore veins once again and (this time in the direction of increase).
- Abolished a separate data pack with extra tags (now it's part of **Uncraftables**).
- Added partially emissive textures to ore creepers. Slightly increased their spawnrate.
...
- Fixed **EMI**'s duplicating mod name tooltips. Fixed periodic crash when using quick crafting feature on Shift+click on the **EMI** panel. Changed default configuration.
- Increased the cost of ammo recipes in **MrCrayfish's Gun Mod**. Now they are more expensive than before, but still 1.5-2 times cheaper than in **[TaCZ] Timeless and Classics Zero**.
- Now the graphics menu has become even more configurable, thanks to the integrations of the new **Sodium Options API**.
- Other minor changes in the configs.
- Removed a lot of legacy recipes that didn't make sense and/or were too unbalanced.
- Removed the creepers' X-ray vision. In return, increased their view range.
- Restored the default weapons pack from **[TaCZ] Timeless and Classics Zero**. (Oops... the purge swept away what shouldn't have been)
- Slightly optimized **Sculk Horde** config. The spread was slowed down from a twentieth of a second to a quarter of a second.
- Tweaked **The Hordes** config. Added all vanilla-looking endermen and illagers from **It Takes a Pillage** to zombie-infectable list and removed **Illager Invasion**'s Necromancer from it. Now villagers can turn into zombies at any difficulty level. Greatly reduced the duration of the `Immunity` effect on food (now it's equal to the duration of the `Regeneration` effect). Returned zombie players. Completely removed the 'protection against zombie infection' tooltips on armor items, because they appear just everywhere.
- Updated the icons of vanilla mobs.

### ⚠ Notes and known issues:
- ⚠ When selecting any of the **Complementary + Euphoria Patches** shaders, a glitch may occur in which the LODs of **Distant Horizons** are overlaid on top of the vanilla chunks. The problem can be solved by switching to a different set of shaders and back, or by resetting the shaderpack settings and then restoring them.
- ¹ Due to the addition of the **Euphoria Patches** mod, all overwritten shader files available in the modpack (also including **Rethinking Voxels**) have been reset and are now loaded from **Modrinth**. This means that when using the **Rethinking Voxels**, **Complementary Reimagined** or **Complementary Unbound** shaders, all modded ore blocks will no longer have emissive textures. However, when using shaders with **Euphoria Patches**, many glowing ores will remain.
- Currently, the rendering of weapons from **[TaCZ] Timeless and Classics Zero** in the first person is broken due to the newly appeared incompatibility with the **First-person Model**. As a temporary fix, I added the general ID of all firearms from this mod (`tacz:modern_kinetic_gun`) to the exception list so that the holding weapons is 'Vanilla'.
- In some recipes in **EMI**, where tags are present, technical lines like `#modid:item_group` are shown without any localization. (I don't see any way to localize this in an organized way, except manually, which I don't want to do because of the incredibly huge number of such tags)
...
### 📋 Long-term plans
- Perhaps **Quark** & **The Aether** with its popular addons may be added in the future.